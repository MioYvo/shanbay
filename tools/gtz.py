...
# 统一的时区处理类
# 原版的timezone需要依赖django，新版本重写所有方法并尽量减少依赖
...
class TimeZone(object):
    ...
    @classmethod
    def local_now(cls, return_str=False):
        _ = cls.naive_to_aware(datetime.datetime.now(), tz=cls.DEFAULT_TIMEZONE)
        if not return_str:
            return _
        else:
            return cls.datetime_to_str(_)

    @classmethod
    def d2t(cls, d, to_utc=True):
        """
        datetime obj 转换为时间戳 (datetime to timestamp)
        :param to_utc:
        :param d: datetime obj
        :return 本地时间，精确到毫秒, POSIX时间戳*1000: 1460364671372
        """
        if cls.is_naive(d):
            # 不带时区信息的,转换为北京时间
            if to_utc:
        ...
        return int((d - datetime.datetime.fromtimestamp(0, tz=pytz.utc)).total_seconds() * 1e3)

    @classmethod
    def t2d(cls, t, to_utc=True):
        """
        时间戳转换为datetime obj (timestamp_to_datetime)
        :param to_utc:
        :param t: timestamp, 精确到毫秒, 时间戳*1000: 1460364671372
        :return: datetime obj 本地时间
        """
        tz = pytz.utc if to_utc else None
        return cls.naive_to_aware(datetime.datetime.fromtimestamp(int(t) / 1e3), tz=tz)
    ...
    def year_range(cls, year, value=None, tz=pytz.utc):
        # type: (int, int, int, int) -> (datetime, datetime)
        """
        计算一年的时间区间, 得到两个utc_datetime元素的列表
        :param year: 年
        :param value: local_datetime, 覆盖参数year
        :param tz: 目标时区
        :return: [utc_year_start_datetime, utc_year_end_datetime]
        """
        # 传入的日期是本地时区
        if value:
            year = value.year
    ...
    def month_range(cls, year=0, month=0, value=None, tz=pytz.utc):
        # type: (int, int, int, int) -> (datetime, datetime)
        """
        计算一个月的时间区间, 得到两个utc_datetime元素的列表
        :param year: 年
        :param month: 月
        :param value: local_datetime, 覆盖参数year, month
        :param tz: 目标时区
        :return: [utc_month_start_datetime, utc_month_end_datetime]
        """
        # 传入的日期是本地时区
        if value:
    ...
    def day_range(cls, year=0, month=0, day=0, value=None, tz=pytz.utc):
        # type: (int, int, int, int) -> (datetime, datetime)
        """
        计算一天的时间区间, 得到两个utc_datetime对象的列表
        :param year: 年
        :param month: 月
        :param day: 日
        :param value: local_datetime, 覆盖参数year, month, day
        :param tz: 目标时区
        :return: [utc_day_start_datetime, utc_day_end_datetime]
        """
        # 传入的日期是本地时区
        if value:
